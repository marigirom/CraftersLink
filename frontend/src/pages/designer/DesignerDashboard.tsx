...
const DesignerDashboard: React.FC = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // Fetch designer's commissions with designer filter
      const commissionsRes = await api.get(`/commissions/?designer=${user.id}&limit=5`);
      const commissions = commissionsRes.data.results || commissionsRes.data || [];
      setRecentCommissions(commissions);

      // Calculate stats with safe fallbacks
      const commissionsSent = commissions.length || 0;
      const pendingConfirmations = commissions.filter((c: Commission) => c.status === 'pending').length || 0;
      const activeProjects = commissions.filter((c: Commission) => c.status === 'in_progress' || c.status === 'accepted').length || 0;
      const completedProjects = commissions.filter((c: Commission) => c.status === 'completed').length || 0;
  ...
  const handleDownloadInvoice = async (commissionId: number) => {
    try {
