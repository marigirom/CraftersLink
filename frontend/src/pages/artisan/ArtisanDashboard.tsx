...
const ArtisanDashboard: React.FC = () => {
  ...
  useEffect(() => {
    // Only fetch data if user and user.id are available
    if (user && user.id) {
      fetchDashboardData();
    }
  }, [user]);
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      if (!artisanId) {
        setError('profile_not_found'); // Special error code
        setLoading(false);
        return;
      }

      // Fetch artisan's products with artisan filter
      const productsRes = await api.get(`/artisans/products/?artisan=${artisanId}`);
      const products = productsRes.data.results || productsRes.data || [];

      // Fetch commissions for this artisan
      const commissionsRes = await api.get(`/commissions/?artisan=${artisanId}&limit=5`);
      const commissions = commissionsRes.data.results || commissionsRes.data || [];
      setRecentCommissions(commissions);

      // Calculate stats with safe fallbacks
      const newRequests = commissions.filter((c: Commission) => c.status === 'pending').length;
      const activeJobs = commissions.filter((c: Commission) => c.status === 'in_progress' || c.status === 'accepted').length;
      const completedJobs = commissions.filter((c: Commission) => c.status === 'completed').length;
  ...
  const handleAcceptCommission = async (commissionId: number) => {
    try {
      await api.patch(`/commissions/${commissionId}/`, { status: 'accepted' });
  ...
  const handleDeclineCommission = async (commissionId: number) => {
    try {
      await api.patch(`/commissions/${commissionId}/`, { status: 'cancelled' });
      fetchDashboardData(); // Refresh data
    } catch (err) {
      console.error('Failed to decline commission:', err);
    }
  };
        ...
        <div className="max-w-3xl mx-auto">
          <div className="bg-amber-50 border-l-4 border-amber-500 p-8 rounded-lg shadow-md">
            <div className="flex items-start mb-6">
              <svg className="w-8 h-8 text-amber-500 mr-4 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
              </svg>
              <div>
                <h2 className="text-xl font-bold text-amber-900 mb-2">Complete Your Profile</h2>
                <p className="text-amber-800 mb-4">
                  Welcome to CraftersLink! To access your dashboard and start listing products, you need to complete your artisan profile first.
                </p>
                <p className="text-amber-700 text-sm mb-6">
                  Your profile helps designers discover your unique crafts and connect with you for custom commissions.
                </p>
                <Link
                  to="/artisan/profile"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-xl shadow-md border border-gray-100 mb-8">
          <div className="p-6 border-b border-gray-200">
            <h2 className="text-xl font-bold text-gray-900">Recent Commission Requests</h2>
          </div>
                ...
                {recentCommissions.map((commission) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <span className={`px-3 py-1 text-xs font-semibold rounded-full border ${getStatusColor(commission.status)}`}>
